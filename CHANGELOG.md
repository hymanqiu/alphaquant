...
| 版本 | 日期 | 类型 | 变更摘要 |
|------|------|------|----------|
| v0.4.0 | 2026-04-22 | feat | 事件影响分析 + 文档体系重构：两步 LLM 筛选重大新闻 → 自动调整 DCF 参数 → 重算内在价值。ARCHITECTURE.md 拆分为三层文档 (nodes/ + decisions/)，8 个节点文档全面重写 (含 I/O 结构体 + NVDA 示例)，新增 MVP-GAP 差距分析 |
